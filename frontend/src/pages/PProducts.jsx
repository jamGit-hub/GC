    ...
    return (
        ...
        >
            ...
            className="product-card"
                ...
                <img
src={getPlatformLogo(p.platform)}
                   alt={p.name}
                style={{ height: "100px",  width:"160px",objectFit: "contain" }} />
<hr/>
                <p style={{ fontWeight: "bold" }}>{p.name}</p>
                <p>{p.region}</p>
                <p >SAR {p.price}</p>

                <button onClick={() => handleAddToCart(p.id)} style={{ background: '#1e1e2f', color: '#BFC9D1', border: 'none', padding: '10px 15px', borderRadius: '4px', cursor: 'pointer', marginTop: 'auto', fontWeight: 'bold' }}>
              Add to Cart
            </button>
            </div>
            ))}
